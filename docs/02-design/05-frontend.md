...
### 5.4 상위/하위 추적성 트리 뷰 (신규 — 다단계 추적성)

기존 04-api.md §4.7 매트릭스 뷰는 "요구사항 x 이슈"를 평면 그리드로 보여주는 데는 적합하지만, 요구사항이 여러 단계로 상위/하위 계층을 이루고 각 단계마다 연결된 이슈가 있는 경우 전체 구조를 한눈에 보기 어렵다. 업계에서 흔히 쓰이는 다단계 추적성 트리 개념을 참고해 아래 트리 뷰를 추가한다.
...
## v3 확장 화면 (2026-08-08, 아직 미구현 — ADR-008, 02-competitive-reference.md 저작권 준수 원칙 먼저 확인할 것)

### 5.11 리뷰 사이클 위젯
요구사항/이슈 상세 화면(§5.2)에 "리뷰" 서브탭을 추가한다. 참여자 목록을 아바타+결정 상태(대기/승인/반려/의견만) 뱃지로 표시하고, 본인이 참여자면 결정 입력 폼을 보여준다. 승인 워크플로우(§5.10 승인함)와는 별도 UI 요소로, 혼동하지 않도록 "리뷰(참고용 의견 수집)"과 "승인 요청(상태 전이 게이트)"이라는 라벨을 명확히 구분해서 표기한다. → 04-api.md §4.17

### 5.12 베이스라인 목록/생성/비교
프로젝트 메뉴에 "베이스라인" 화면 추가. 목록에서 베이스라인 생성(요구사항/이슈/테스트케이스를 검색해 담기) 및 이름/설명 입력. 상세 화면에서 "현재와 비교" 버튼 클릭 시 필드별 변경분(추가됨/삭제됨/값 변경됨)을 색상으로 구분해 보여준다(예: 삭제된 값은 취소선, 추가된 값은 강조). → 04-api.md §4.18

### 5.13 위험 관리 보드
프로젝트 메뉴에 "위험 관리" 화면 추가. 기본 뷰는 발생가능성×영향도 3x3 격자(히트맵) 형태로 위험을 배치해서 보여주고(격자 셀 클릭 시 해당 조합의 위험 목록 필터), 목록 뷰로 전환도 가능하게 한다. 위험 상세 화면에서 완화 계획 편집 + 연결된 요구사항/이슈 표시(§4.7 추적성 링크 재사용). → 04-api.md §4.19

### 5.14 요구사항 문서 뷰 + 변형 필터
요구사항 목록 화면(§5.2)에 "문서 뷰" 토글을 추가한다. 켜면 표/카드 목록 대신 상위 요구사항부터 순서대로(order_index 기준) 들여쓰기된 문서 목차 형태로 렌더링(마크다운 문서를 읽는 느낌, 다만 리치 텍스트 편집은 지원하지 않음 — 01-scope.md §1.3 v3 비스코프 참고). 상단에 변형 선택 드롭다운을 두어, 특정 변형을 선택하면 그 변형에서 EXCLUDED로 표시된 요구사항은 흐리게 표시하거나 숨긴다. 같은 부모를 가진 요구사항끼리는 드래그로 순서 변경 가능(→ `PATCH .../order`). → 04-api.md §4.20

### 5.15 대시보드 위젯 커스터마이징 + 내보내기 버튼
프로젝트 대시보드(§5.2)와 "내 작업" 개인화 대시보드에 "위젯 편집" 모드를 추가해, 위젯을 추가/제거/순서 변경할 수 있게 한다(드래그 정렬). 추적성 매트릭스(§5.2)와 테스트런 실행 화면(§5.7)에 "내보내기" 버튼을 추가해 Excel/PDF를 선택해 다운로드할 수 있게 한다. → 04-api.md §4.21
